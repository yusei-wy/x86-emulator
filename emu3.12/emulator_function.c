...
void set_register8(Emulator *emu, int index, uint8_t value) {
  if (index < 4) {
    ...
    emu->registers[index] = r | ((int32_t)value << 8);
  }
}
...
void update_eflags_sub(Emulator *emu, uint32_t v1, uint32_t v2,
                       uint64_t result) {
  // 角値の符号を取得
  // 符号付き 32bit においては 31bit 目は符号ビット
  int sign1 = v1 >> 31;
  int sign2 = v2 >> 31;
  int signr = (result >> 31) & 1;

  // 演算結果に carry があれば Carry フラグ設定
  set_carry(emu, result >> 32);

  // 演算結果が-1ならば Zero フラグ設定
  set_zero(emu, result == 0);

  // 演算結果に符号があれば Sign フラグ設定
  set_sign(emu, signr);

  // 演算結果がオーバーフローしていたら Overflow フラグ設定
  /*
    2つの符号付き32ビット整数の符号が同じ時、つまり両方とも0以上の数か
    両方とも0未満のとき、演算結果は必ず32ビット整数として表せる

    [Overflow] 正 - 負 = 正 + 正, 負 - 正 = 負 + 負
  */
